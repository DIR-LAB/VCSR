...
void Compress(const WGraph &g, pvector<NodeID>& comp) {
...
pvector<NodeID> Afforest(const WGraph &g, int32_t neighbor_rounds = 2) {
...
#pragma omp parallel for schedule(dynamic, 16384)
    for (NodeID u = 0; u < g.num_nodes(); u++) {
      ...
      for (WNode v : g.out_neigh(u, neighbor_rounds)) {
        if(v.w == 0) continue;
        Link(u, v.v, comp);
      }
    }
  ...
  for (auto label_source_pair : label_to_source) {
    ...
    for (auto it = frontier.begin(); it != frontier.end(); it++) {
      ...
      for (WNode v : g.out_neigh(u)) {
        if(v.w == 0) continue;
        if (comp[v.v] != curr_label)
          return false;
        if (!visited.get_bit(v.v)) {
          visited.set_bit(v.v);
          frontier.push_back(v.v);
        }
      }
      if (g.directed()) {
        for (WNode v : g.in_neigh(u)) {
...
int main(int argc, char* argv[]) {
  ...
  auto CCBound = [](const WGraph& gr){ return Afforest(gr); };
