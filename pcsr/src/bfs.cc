  ...
  #pragma omp parallel
  {
    ...
    for (auto q_iter = queue.begin(); q_iter < queue.end(); q_iter++) {
      ...
      for (WNode v : g.out_neigh(u)) {
        if(v.w == 0) continue;
        NodeID curr_val = parent[v.v];
        if (curr_val < 0) {
          if (compare_and_swap(parent[v.v], curr_val, u)) {
            lqueue.push_back(v.v);
  ...
  for (NodeID u : g.vertices()) {
    if ((depth[u] != -1) && (parent[u] != -1)) {
      ...
      for (WNode v : g.in_neigh(u)) {
        if(v.w == 0) continue;
        if (v.v == parent[u]) {
          if (depth[v.v] != depth[u] - 1) {
            cout << "Wrong depths for " << u << " & " << v.v << endl;
            return false;
          }
          parent_found = true;
          break;
