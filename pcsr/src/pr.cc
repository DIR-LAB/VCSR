...
pvector<ScoreT> PageRankPull(const WGraph &g, int max_iters,
                             double epsilon = 0) {
  ...
  for (int iter=0; iter < max_iters; iter++) {
    ...
    for (NodeID u=0; u < g.num_nodes(); u++) {
      ...
      for (WNode v : g.in_neigh(u)) {
        if(v.w == 0) continue;
        incoming_total += outgoing_contrib[v.v];
      }
...
//   error < target_error
bool PRVerifier(const WGraph &g, const pvector<ScoreT> &scores,
                        double target_error) {
  const ScoreT base_score = (1.0f - kDamp) / g.num_nodes();
  ...
  for (NodeID u : g.vertices()) {
    ...
    for (WNode v : g.out_neigh(u)) {
      if(v.w == 0) continue;
      incomming_sums[v.v] += outgoing_contrib;
    }
  }
