...
class WriterBase {
 public:
  ...
  void WriteHammerGraph(std::string filename, bool serialized = false) {
    if (filename == "") {
      std::cout << "No output filename given (Use -h for help)" << std::endl;
      std::exit(-8);
    }
    ...
    bool *flag = (bool *) calloc(g_.num_nodes(), sizeof(bool));

    for (NodeID_ uu=0; uu < NODE_GAP; uu+=1) {
      for (NodeID_ u = uu; u < g_.num_nodes(); u += NODE_GAP) {
        if(flag[u]) {
          std::cout << "Neighbors of " << u << " already written to the file!" << std::endl;
          std::exit(-1);
        }
        flag[u] = 1;
        for (DestID_ v : g_.out_neigh(u)) {
          file << u << " " << v.v << " " << v.w << std::endl;
        }
      }
    }
  ...
  void WriteHammerExtremeGraph(std::string filename, bool serialized = false) {
    if (filename == "") {
      std::cout << "No output filename given (Use -h for help)" << std::endl;
      std::exit(-8);
    }
    ...
    for (NodeID_ u = 0; u < g_.num_nodes(); u += 1) {
      for (DestID_ v : g_.out_neigh(u)) {
        file << u << " " << v.v << " " << v.w << std::endl;
      }
    }
